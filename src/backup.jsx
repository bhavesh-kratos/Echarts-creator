          ...
          dataset.dimensions.map(data => {
            if (data !== xAxisDimension)
              return {
                ...
                // encode: {
                //   'y': this.state.dataInfo.reduce((acc, dat) => {
                //     return dat.type === 'NUMBER' ? [...acc, dat.name] : acc
                //   }, [])
                // }
              }
          })
      ...
      series:
        dataset.dimensions.map(dat => {
          ...
          return {
            type: 'scatter',
            symbolSize:  (data) => {
              return !!this.state.yAxisDimension ? Math.sqrt(data[this.state.yAxisDimension]) / 5e2 : 1;
            },
            label: {
              emphasis: {
                show: true,
  ...
  this.setState({
    ...
    switch (this.state.chartType) {
      case 'line':
        ...
        setOptions('tooltip', ['formatter'], "{a}<br/>{b} : {c}");
        break;
      case 'bar':
        this.setState({
          options: newOptions,
...
class ChartCreator extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      dimensions: ['product', '2015', '2016', '2017', 'date'],
      dataInfo: [{ name: 'product', type: 'STRING' }, { name: '2015', type: 'NUMBER' }, { name: '2016', type: 'NUMBER' }, { name: '2017', type: 'NUMBER' }, { name: 'date', type: 'TIME' }]
    })
  }
  ...
  chartInitialization = (dataset, theme = null) => {
    ...
    let chart = echart.init(document.getElementById("main"), !!this.state.theme ? this.state.theme : 'default');
    this.setState({
      chart
    }, () => {
      ...
      <div>
        ...
        <h3>Color theme: </h3> &nbsp;
          ...
          onChange={(e, { value }) => {
            this.setState({
              ...
              this.chartInitialization(this.state.options.dataset, value)
            })
          }}
          options={
        ...
        <Dropdown
          ...
          onChange={(event, { value }) => this.setState({ xAxisDimension: value }, () => {
            // this.setOptions(null, ['dataset', 'dimensions'], Array.from(new Set([this.state.xAxisDimension, ...this.state.options.dataset.dimensions])))
            this.setOptions('series', ['encode', 'x'], value)
            this.chartInitialization(this.state.options.dataset)
          })}
          options={
            !!this.state.dataInfo && !!this.state.dataInfo && _map(this.state.dataInfo.filter(dat => dat.name !== this.state.yAxisDimension && this.dimensionOptions('x', this.state.chartType, dat.type)), (key) =>
        ...
        <Dropdown
          ...
          onChange={(event, { value }) => this.setState({ yAxisDimension: value }, () => {
            this.setOptions('series', ['encode', 'y'], value)
          ...
          options={
            !!this.state.dataInfo && !!this.state.dataInfo && _map(this.state.dataInfo.filter(dat => dat.name !== this.state.yAxisDimension && this.dimensionOptions('y', this.state.chartType, dat.type)), (key) =>
              ({ key: key.name, value: String(key.name), text: key.name }))} />
        <h3>Orientation: </h3> &nbsp;
        <Dropdown
        ...
        <Sidebar.Pushable as={Segment} style={{ background: '#333', color: 'white', overflowY: 'hidden' }}>
          ...
          <Sidebar.Pusher >
            <div className="chart-pusher">
              ...
              >
                <div id="main" style={{ background: 'white', marginLeft: '20px' }} />
              </div>
            </div>
          </Sidebar.Pusher>
